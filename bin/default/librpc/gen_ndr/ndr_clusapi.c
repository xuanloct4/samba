...
static enum ndr_err_code ndr_pull_clusapi_AddNotifyResourceTypeV2(struct ndr_pull *ndr, int flags, struct clusapi_AddNotifyResourceTypeV2 *r)
{
	...
	if (flags & NDR_IN) {
		...
		size_resTypeName_1 = ndr_get_array_size(ndr, &r->in.resTypeName);
		length_resTypeName_1 = ndr_get_array_length(ndr, &r->in.resTypeName);
...
static enum ndr_err_code ndr_pull_clusapi_ChangeCsvStateEx(struct ndr_pull *ndr, int flags, struct clusapi_ChangeCsvStateEx *r)
{
	...
	if (flags & NDR_IN) {
		...
		size_lpszVolumeName_1 = ndr_get_array_size(ndr, &r->in.lpszVolumeName);
		length_lpszVolumeName_1 = ndr_get_array_length(ndr, &r->in.lpszVolumeName);
