...
_PUBLIC_ enum ndr_err_code ndr_pull_svcctl_ChangeServiceConfigW(struct ndr_pull *ndr, int flags, struct svcctl_ChangeServiceConfigW *r)
{
	...
	if (flags & NDR_IN) {
		...
		if (r->in.dependencies) {
			NDR_CHECK(ndr_check_array_size(ndr, (void*)&r->in.dependencies, r->in.dwDependSize));
		}
		if (r->in.dependencies) {
			NDR_CHECK(ndr_check_array_length(ndr, (void*)&r->in.dependencies, r->in.dwDependSize));
		}
		if (r->in.password) {
			NDR_CHECK(ndr_check_array_size(ndr, (void*)&r->in.password, r->in.dwPwSize));
		}
		if (r->in.password) {
			NDR_CHECK(ndr_check_array_length(ndr, (void*)&r->in.password, r->in.dwPwSize));
		}
	}
	if (flags & NDR_OUT) {
#ifdef FUZZING_BUILD_MODE_UNSAFE_FOR_PRODUCTION
